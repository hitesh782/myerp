...
public class QueryBuilder {
	...
	private static Query buildQuery(String key, FilterMetadata value, Query query) throws ParseException {
		...
		case CONTAINS:
			query.addCriteria(Criteria.where(key).regex(Pattern.compile(Pattern.quote(String.valueOf(value.getValue())),
					Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE)));
			break;
		case GTE:
			query.addCriteria(Criteria.where(key).gte(Double.valueOf(String.valueOf(value.getValue()))));
			break;
		case LTE:
			query.addCriteria(Criteria.where(key).lte(Double.valueOf(String.valueOf(value.getValue()))));
			break;
		case GT:
			query.addCriteria(Criteria.where(key).gt(Double.valueOf(String.valueOf(value.getValue()))));
			break;
		case LT:
			query.addCriteria(Criteria.where(key).lt(Double.valueOf(String.valueOf(value.getValue()))));
			break;
		case IS:
			query.addCriteria(Criteria.where(key).is(value.getValue()));
			break;
